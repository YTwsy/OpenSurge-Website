...
export async function generateMetadata({ params }: PageProps<"/[section]">) {
  ...
  const copy = sectionCopy.en[section];
  return createMetadata({
    ...
    keywords: ["OpenSurge", `OpenSurge ${copy.label}`, "macOS gateway"],
  });
}

export default async function EnglishSection({ params }: PageProps<"/[section]">) {
  const { section } = await params;
  if (!isSectionKey(section)) notFound();
  return <SectionLanding locale="en" section={section} />;
}
